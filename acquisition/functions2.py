...
IntTime = 0.1
def extract_intensities(viddirs,vidfiles,imfiles,coords,inference_coords,imdir):
    for video in range(len(vidfiles)):
        ...
        x_offset = 512/inference_coords[0,1]
        y_offset = 512/inference_coords[0,2]
        txt = open("".join(glob.glob(os.path.join(imdir + "*" + str(0).zfill(3) + "_img_cp_outlines.txt"))),"r")
        with txt:
            ROIlist = [line.rstrip('\n') for line in txt]
            ...
            for ROIs, line in enumerate(txt):   
                xy = map(int, line.rstrip().split(","))
                X = list(xy)[::2]
                X_points = [i * x_offset for i in X]
                #X_points = list(np.subtract(np.array(X),x_offset))
                xy = map(int, line.rstrip().split(","))
                Y = list(xy)[1::2]
                #Y_points = list(np.add(np.array(Y),y_offset))
                Y_points = [i * y_offset for i in Y]
                ...
                roi_vertices = np.array([list(zip(X_points,Y_points))]) ## prepare coordinate data for vertices of ROI
    ...
    for video in range(len(vidfiles)):
        ...
        if os.path.isfile("".join(glob.glob(os.path.join(imdir + "*" + 
            ...
            for ROIs, line in enumerate(txt):   ### Adapted from inference machine
                ...
                coordata[ROIs,0:3] = ROIs,X_centroid * x_abs,Y_centroid * y_abs
...
def get_coords(viddir,coords,imdir,**kwargs):
    ...
    return coords
def get_intensities(viddir,coords,inference_coords,**kwargs):
    os.chdir(viddir)
    if "imdir" in kwargs:
        imdir = kwargs["imdir"]
    else:
    ...
    extract_intensities(vidfiles, viddirs, imfiles,coords,inference_coords[0,1:3],imdir)
