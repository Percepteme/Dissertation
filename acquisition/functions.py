...
def rename_videos(rename_videos,viddirs_raw):
...
def create_max_intensity(viddirs, vidfiles,rename_videos):
    ...
    for video in range(len(vidfiles)):
        ...
        with txt:
            ...
            for ROIs, line in enumerate(txt):   
                xy = map(int, line.rstrip().split(","))
                X = list(xy)[::2]
                X_points = np.subtract(np.multiply(np.divide(np.array(X),1024),micro_dimensions),x_offset)
                xy = map(int, line.rstrip().split(","))
                Y = list(xy)[1::2]
                Y_points = np.subtract(np.multiply(np.divide(np.array(Y),1024),micro_dimensions),y_offset)
                ...
                roi_vertices = np.array([list(zip(X/1024*micro_dimensions,Y/1024*micro_dimensions))]) ## prepare coordinate data for vertices of ROI
...
def extract_coords(viddirs,vidfiles,imfiles, coords):
    ...
    for video in range(len(vidfiles)):
        ...
        if os.path.isfile("".join(glob.glob(os.path.join(imdir + "*" + 
            ...
            for ROIs, line in enumerate(txt):   ### Adapted from inference machine
                ...
                coordata[ROIs,0:3] = ROIs,(X_centroid/1024 * micro_dimensions) - x_offset,(Y_centroid/1024 * micro_dimensions) - y_offset
...
def get_coords(viddir,coords,**kwargs):
    ...
    create_max_intensity(viddirs,vidfiles,rename_videos=False)
    
    imfiles = get_imglist(imdir)
    
    inference(imfiles)
    
    coords = extract_coords(vidfiles,viddirs,imfiles,coords)
...
def get_intensities(viddir,coords,**kwargs):
    ...
    extract_intensities(vidfiles, viddirs, imfiles,coords)
