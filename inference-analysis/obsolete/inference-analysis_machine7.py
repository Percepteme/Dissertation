...
for image in range(len(imfiles)):
    ...
    if rename_videos == "False":
        if os.path.isfile(os.path.join(imdir + im + "_cp_outlines.txt"))==False:
...
def show_graph_with_labels(adjacency_matrix,coordata,thresh,**kwargs): # takes an adjacency matrix, as above and a threshold coefficient
    ...
    x = np.array(pg.pairwise_corr(amatrix))
    weight_data = np.zeros(x[:,0].shape)
    weight_data = x[:,5].astype(int)
    ...
    for i in G.edges():
        G[i[0]][i[1]] = weight_data[i]
    
    pos = {coordata[i,0]:tuple(coordata[i,1:]) for i in range(len(coordata))}
    nx.set_node_attributes(G, pos, 'coord')
    nx.draw(G, node_size=0, labels=None, with_labels=False,edge_color="red", style='--')
    ax = plt.gca()
...
def maproi_to_image(mode,dat_num,**kwargs): 
    image = os.path.join(str(dat_num).zfill(3) + "_img.png") 
    ...
    if mode == "graph":
        ...
        #fig.patch.set_facecolor("black")
        plt.imshow(plt.imread(image))
    plt.savefig(os.path.join(imdir + 'labelled-masks_' + str(video).zfill(3) + '.pdf'))
    plt.close()
    if check == True:
        return insideROS
...
for video in range(len(vidfiles)):
        ...
        with txt:
            ...
            for ROIs, line in enumerate(txt):   
                ...
                if rename_videos == "True":
                    ...
                    save = np.savetxt(os.path.join(imdir + '/' + os.path.splitext(os.path.basename(str(viddirs_raw[video])))[0] + "_" + str(video).zfill(3) + '_intensities.csv'), output_tab,delimiter=",")
    ...
    plt.ylim(-0.5,2)
    plt.savefig(os.path.join(imdir + 'dFNormF0_' + str(video).zfill(3) + '.pdf'))
    plt.close()
    ...
    plt.ylim(-0.5, 2)
    plt.savefig(os.path.join(imdir + 'dFNormF0-grid_' + str(video).zfill(3) + '.pdf'))
    plt.close()
