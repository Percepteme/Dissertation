...
mdir = args.mdir
model= models.CellposeModel(pretrained_model=mdir, gpu=True, diam_mean=30)
...
for i in range(len(vidfiles)):
    if rename_videos == "False":
        ...
        if os.path.isfile(os.path.join(imdir + imname
                                       + "_" + str(i).zfill(3) + "_img" + imformat))==False:
        ...
        if os.path.isfile(os.path.join(imdir + os.path.splitext(str(i).zfill(3) + "_img" + imformat)))==False:
            IM = io.imread(viddirs[i])
            IM_MAX= np.max(IM, axis=0)
...
for video in range(len(vidfiles)):
    if os.path.isfile(os.path.join(viddir + '/' + str(video).zfill(3) + '_intensities.csv'))==True:
        pass
    else:
