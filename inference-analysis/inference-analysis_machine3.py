...
parser.add_argument("--mdir",
                    type=str, required=False, default="/home/caiusgibeily/Downloads/Training-Optocamp/training/models/cellpose_residual_on_style_on_concatenation_off_training_2022_03_26_13_48_54.341824", 
                    help="This is where one sets the directory to the pretrained model")
parser.add_argument("--viddir",
                    type=str, required=True,default="/home/caiusgibeily/Downloads/Training-Optocamp/test/",
                    help="This where the input directory to the videos is specified. WARNING - the script will rename files numerically")
parser.add_argument("--save_im",
                    required=False,
                    help="Choose whether to save the pipeline images (these show original image, outlines, masks and predicted cell poses). Set True to save in imdir")
parser.add_argument("--imdir",
                    type=str,required=True, default = "/home/caiusgibeily/Downloads/Training-Optocamp/test/",
                    help="This is where the output directory to the images is specified")
parser.add_argument("--imformat",
                    type=str,required=False,choices=[".png",".jpg",".tif"],default=".png",
                    help="Choose the extension type of your images for segmentation. Default = .png")
parser.add_argument("--dia",
                    type=int,required=False, default=30,
                    help="Select an average diameter value for cellpose inference. Default = 30")
parser.add_argument("--mask_thresh",
                    type=float,required=False,default=0.0,
                    help="Select a mask threshold to alter sensitivity of mask detection. \nDefault = 0.0")
parser.add_argument("--rename_videos",
                    required=False,choices=["True","False"],default="False",
                    help="Choose whether to rename entire stem of video to numeric system: 000, 001 etc. Default = False")
parser.add_argument("--subdir", 
                    required=False,choices=["True","False"],default="False",
                    help="Choose whether the software should search the subdirectories in the video directory for video files")
...
parser.add_argument("--spider", 
                    required=False,choices=["True","False"],default="False",
                    help="Choose whether the software should search the subdirectories in the video directory for video files")
parser.add_argument("--graph_mask", 
                    required=False,choices=["True","False"],default="False",
                    help="Choose whether the software should search the subdirectories in the video directory for video files")
parser.add_argument("--same_fovs", 
                    required=False,choices=["True","False"],default="False",
                    help="Choose whether the software should search the subdirectories in the video directory for video files")
parser.add_argument("--check_correlations", 
                    required=False,choices=["True","False"],default="False",
                    help="Choose whether the software should search the subdirectories in the video directory for video files")
...
############# Correct for variance in predicted cellpose masks - due to differences in fluorescence changes and background noise, the algorithm
#### may identify a different number of FOVs per experiment. Hence, this code will identify, for any number of FOVs and any repeat experiment per FOV, the
#### maximum detected ROIs and copy these coordinate data to all other experiments within the FOV. Insofar as the algorithm has a 0% false positive rate, 
#### this is ideal for maximum detection and consistency across experiments
same_fovs = args.same_fovs
...
if same_fovs == "True":
    ...
    for i in range(len(imfiles)):
        fov = int(re.search("FOV_(.+?)-",imfiles[i]).group(1))
        allfov.append(fov)
    numfov = np.unique(allfov)
    #### Quantify number of ROIs per inference
    for j in range(len(numfov)):
        maxROI = []
        for k in range(len(allfov)):
            file = "".join(glob.glob(os.path.join(imdir + "*" + str(k).zfill(3) + "_img_cp_outlines.txt")))

            if int(re.search("FOV_(.+?)-",file).group(1)) == numfov[j]:        
                txt = open(file,"r")
                ROIlist = [line.rstrip('\n') for line in txt]    
                ROIs = len(ROIlist)
                maxROI.append(ROIs)
                ...
                maxROI.append(0)
        index = maxROI.index(max(maxROI))
            ### replace files
        for l in range(len(imfiles)):
            if int(re.search("FOV_(.+?)-",imfiles[l]).group(1)) == numfov[j] and l != index:     
                maskee = os.path.splitext(imfiles[l])[0] + "_cp_outlines.txt"
                masker = os.path.splitext(imfiles[index])[0] + "_cp_outlines.txt"
                ## mask replacement
                shutil.copy(masker,maskee)
                shutil.copy(imfiles[index],imfiles[l])
            else:
                pass
...
def maproi_to_image(mode,dat_num,**kwargs): 
    ...
    for ROIs, line in enumerate(txt):   ### Adapted from inference machine
        ...
        coordata[ROIs,1] = X[int(len(X)/2)]
        coordata[ROIs,2] = Y[int(len(Y)/2)]    
        ...
        if mode == "graph_mask":
            #plt.plot(boolean[1],boolean[0], color="red") # plot X and Y masks
            plt.plot(boolean[1],boolean[0],c="green")
        elif mode == "mask":
            if check == True:
    ...
    if mode == "graph":
        show_graph_with_labels(kwargs["cordata"],coordata=coordata,thresh=0.8,fill=True)
        plt.savefig(os.path.join(imdir + os.path.splitext(os.path.basename(str(imfiles[video])))[0] + '_spider_graph' + '.pdf'))
    elif mode == "mask":
        #fig.patch.set_facecolor("black")
        plt.imshow(plt.imread(image))
        plt.savefig(os.path.join(imdir + os.path.splitext(os.path.basename(str(imfiles[video])))[0] + '_labelled-masks' + '.pdf'))
    elif mode == "graph_mask":
        show_graph_with_labels(kwargs["cordata"],coordata=coordata,thresh=0.8,fill=True)
        #circ = plt.Circle((kwargs["xcoord"],kwargs["ycoord"]),kwargs["radius"],color="green",fill=False,lw=5)
        #ax.add_patch(circ)
        plt.savefig(os.path.join(imdir + os.path.splitext(os.path.basename(str(imfiles[video])))[0] + '_spider_labelled_graph' + '.pdf'))
    plt.close()
    if check == True:
        return insideROS
...
fos = args.fos
spider = args.spider
graph_mask = args.graph_mask
graph = args.graph
check_correlations = args.check_correlations
correlations = []


for video in range(len(vidfiles)):
    if check_correlations == "False":
        ...
        if os.path.isfile("".join(glob.glob(os.path.join(imdir + "*" + 
            ...
            with txt:
                ...
                for ROIs, line in enumerate(txt):   
                    ...
                    img = skimage.io.imread(imfiles[video],plugin='pil').astype(np.uint16)
                ...
                if rename_videos == "True":
                    ...
                    save = np.savetxt(os.path.join(imdir + os.path.splitext(os.path.basename(str(imfiles[video])))[0] + "_" 
                                                   + str(video).zfill(3) + '_intensities.csv'), output_tab,delimiter=",")   
        ...
        ### DF/F0 plots
            ...
            if spider == "True":
                ...
                correlations.append(round(cordata.stack().mean(),3))
                #sb.set(font_scale=1.2)
                cluster = sb.clustermap(cordata,cmap="plasma",vmin=0,vmax=1) 
                plt.savefig(os.path.join(imdir + os.path.splitext(os.path.basename(str(imfiles[video])))[0] + '_clustermap' + '.pdf'))
                plt.close()
                if os.path.isfile("".join(glob.glob(os.path.join(imdir + "*" + 
                                                                 str(video).zfill(3) + "_spider_graph.pdf"))))==False:
                    maproi_to_image(mode = "graph",cordata=cordata,dat_num=video)
                    plt.close()
            
            if fos == "True":
                    if os.path.isfile("".join(glob.glob(os.path.join(imdir + "*" + 
                                                                     str(video).zfill(3) + "_labelled-masks.pdf"))))==False:
                        FOS = maproi_to_image(mode="mask",checkROS=True,xcoord=512,ycoord=512,radius=116, dat_num=video)
                        save = np.savetxt(os.path.join(imdir + os.path.splitext(os.path.basename(str(imfiles[video])))[0] 
                                                       + "_" + str(video).zfill(3) + '_fos.csv'), FOS,delimiter=",")   
            if graph_mask == "True":
                pdata = pd.DataFrame(DFNormF0)
                cordata = round(pdata.corr(),2)
                #mask = maproi_to_image(mode="graph_mask",cordata=cordata,checkROS=True,xcoord=512,ycoord=512,radius=116,dat_num=video)
                mask = maproi_to_image(mode="graph_mask",cordata=cordata,checkROS=True,xcoord=512,ycoord=512,radius=116,dat_num=video)
                print(cordata.stack().mean())
            
            if graph == "True":
                pdata = pd.DataFrame(DFNormF0)
                cordata = round(pdata.corr(),2)
                mask = maproi_to_image(mode="mask",cordata=cordata,dat_num=video)
            #rasterplot(DFNormF0,showpeaks=False)
            plt.close()
        ...
        output_tab = np.array(pd.read_csv("".join(glob.glob(os.path.join(imdir + "*" + 
                                                         str(video).zfill(3) + "_intensities.csv")))))
        vid = len(output_tab)*IntTime
        # Convert frame number, stored in the first column, to time, by multiplying by IntTime. Start at time = 0 by subtracting IntTime.
        time = np.multiply(output_tab[:,0],IntTime)
        data = output_tab[:,1:]
        ...
        print(round(cordata.stack().mean(),3))
        
correlations = np.array(correlations)
np.savetxt(os.path.join(imdir + "correlation_averages.csv"),correlations)
