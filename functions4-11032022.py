...
parser.add_argument("--mdir",
                    type=str, required=False, default="C:\\Users\\COB OptoCaMP\\Documents\\Inference_machine\\optocamp.849970", help="This is where one sets the directory to the pretrained model")
parser.add_argument("--viddir",
                    type=str, required=False,default="C:\\Users\\COB OptoCaMP\\Documents\\Inference_machine\\videos\\",help="This where the input directory to the videos is specified. WARNING - the script will rename files numerically")
...
parser.add_argument("--imdir",
                    type=str,required=False, default = "C:\\Users\\COB OptoCaMP\\Documents\\Inference_machine\\videos\\",help="This is where the output directory to the images is specified")
...
def inference(imfiles,imdir):    
    ...
    for image in range(len(imfiles)):
        ...
        if os.path.isfile("".join(glob.glob(os.path.join(imdir + "*" + 
            ...
            pass
def inference_rove(imfiles,imdir,image,**kwargs):
...
def enget_intensities(viddirs,vidfiles,imfiles,imdir,coords,inference_coords):
    for video in range(len(vidfiles)):
        ...
        x_offset = 512/inference_coords[1]
        y_offset = 512/inference_coords[2]
        ...
        with txt:
            ...
            for ROIs, line in enumerate(txt):   
                xy = map(int, line.rstrip().split(","))
                #X = list(xy)[::2]
                X = list(map(lambda x: x * x_offset, xy))[::2]
                xy = map(int, line.rstrip().split(","))
                #Y = list(xy)[1::2]
                Y = list(map(lambda x: x * y_offset, xy))[::2]
...
def extract_coords(viddirs,vidfiles,imfiles, coords,imdir):
    ...
    for video in range(len(vidfiles)):
        ...
        if os.path.isfile("".join(glob.glob(os.path.join(imdir + "*" + 
            ...
            micropix = 0.34 # 0.63 um per pixel 
            ...
            return coordata
def extract_coords_rove(viddirs,vidfiles,imfiles, coords,imdir):

        txt = open("".join(glob.glob(os.path.join(imdir + "/" + "*" + "cp_outlines.txt"))),"r")
        ROIlist = [line.rstrip('\n') for line in txt]
        coordata = np.array([[0 for l in range(3)] for m in range((len(ROIlist)))])
...
def extract_ROIs(viddirs,vidfiles,imfiles,imdir):
    txt = open("".join(glob.glob(os.path.join(imdir + "/" + "*" + "_cp_outlines.txt"))),"r")
    c = 0
    with txt:
        ROIlist = [line.rstrip('\n') for line in txt]
        ROIs = len(ROIlist)
...
def get_coords(viddir,coords,imdir,**kwargs):
    os.chdir(viddir)
    ...
    image = kwargs["image"] 
    inference(imfiles,imdir,image)
    if "mode" in kwargs and kwargs["mode"] == "rove":
        coords = extract_coords_rove(vidfiles,viddirs,imfiles,coords,imdir)
    else:
        coords = extract_coords(vidfiles,viddirs,imfiles,coords,imdir)
    #extract_intensities(vidfiles, viddirs, imfiles)
    return coords
def get_intensities(viddir,coords,**kwargs):
    os.chdir(viddir)
    if "imdir" in kwargs:
        imdir = kwargs["imdir"]
    else:
...
def get_ROIs(viddir,image,**kwargs):
    os.chdir(viddir)
    if "imdir" in kwargs:
        imdir = kwargs["imdir"]
    else:
    ...
    inference_rove(imfiles,imdir,image=image)
    
    #extract_coords(vidfiles,viddirs,imfiles)
    ROIs = extract_ROIs(vidfiles, viddirs, imfiles,imdir)
